...
class VietShield_Admin_Dashboard {
    ...
    public function enqueue_assets($hook) {
        ...
        wp_enqueue_script(
            'vietshield-admin-pages',
            VIETSHIELD_PLUGIN_URL . 'admin/js/admin-page-scripts.js',
            ['jquery', 'vietshield-admin'],
            time(),
            true
        );
    ...
    public function ajax_get_file_scan() {
        ...
        if ($result && !empty($result['scan'])) {
            require_once VIETSHIELD_PLUGIN_DIR . 'includes/class-vietshield-helpers.php';
            $result['scan']['started_at'] = \VietShield_Helpers::format_timestamp($result['scan']['started_at'], 'Y-m-d H:i:s');
            $result['scan']['finished_at'] = \VietShield_Helpers::format_timestamp($result['scan']['finished_at'], 'Y-m-d H:i:s');
            
            if (!empty($result['items'])) {
                foreach ($result['items'] as &$item) {
                    $item['file_mtime'] = \VietShield_Helpers::format_timestamp($item['file_mtime'], 'Y-m-d H:i:s');
                }
            }
        }
